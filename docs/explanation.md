# III-V MQW 設計・簡易 k.p ゲイン計算で実施している物理計算の詳細
...
また、`src/MQWGainDesign.py`、`src/MQWGainSweep.py`、`src/FitCalibration.py`、`src/CompareSpectrum.py`
で追加された簡易 k.p 材料ゲイン計算、校正、可視化、fit、外部スペクトル比較の扱いも説明する。
...
- 校正 JSON を用いた band offset、線幅、gain scale の再現可能な適用
- peak、FWHM、RMSE などのスペクトル指標
- Lumerical / 実測などの外部基準スペクトルとの比較
- バンド図、波動関数、サブバンド分散、sweep summary の可視化

## 0.1 現在のコード構成

現行のコードは、役割ごとに次のように分かれている。

| file | role |
| --- | --- |
| `BasicMQWDesign.py` | 材料補間、組成逆算、ひずみ、有限井戸準位、遷移波長、Lumerical `.lsf` 出力 |
| `CriticalFilmStress.py` | 単層の膜応力、応力厚み積、簡易臨界膜厚 |
| `kp_solver.py` | MQW z-grid 展開、スカラー伝導帯、2x2 HH/LH k.p ブロック、`k_t` sweep |
| `gain.py` | 擬フェルミ準位、TE/TM 遷移強度、線幅畳み込み、材料ゲインスペクトル |
| `calibration.py` | 校正 JSON の読み込み、検証、CLI override 解決、出力 summary |
| `MQWGainDesign.py` | 単一設計の k.p ゲイン計算 CLI |
| `MQWGainSweep.py` | 設計値または校正値の sweep CLI |
| `metrics.py` | peak、FWHM、指定波長 gain、スペクトル RMSE などの指標計算 |
| `visualization.py` | gain spectrum、band diagram、wavefunction、dispersion、sweep summary の PNG 生成 |
...
簡素貸したモデルであり、以下は想定していない。そのため、あくまで参考のために用いる用途である。
...
## 0.3 校正、sweep、可視化、fit の位置づけ

`calibration.py` は、校正 JSON から次の値を読み込む。
...
```text
CLI で明示した値 > 校正 JSON の値 > コード側の既定値
```

ただし `MQWGainSweep.py` で `--sweep qc` または `--sweep broadening-eV` を指定した場合は、sweep 値がそのフィールドを最優先で上書きする。
出力 JSON には、校正ファイル由来の base 設定、CLI override の有無、sweep が上書きした校正フィールドが保存される。

`visualization.py` は、数値結果の妥当性を目視確認するための補助である。
主な図は次である。

- gain spectrum: TE/TM 材料ゲインとピーク位置
- band diagram: 相対的な伝導帯 profile、HH/LH edge、`k_t=0` の e/h 準位
- wavefunction: `k_t=0` における電子・正孔波動関数の確率密度
- subband dispersion: `k_t` に対する電子・正孔サブバンド
- sweep summary: sweep 値に対する peak wavelength と peak gain

`metrics.py` は、ゲインスペクトルから peak、FWHM、指定波長 gain、RMSE を取り出すための共通部品である。
`spectrum_io.py` は、計算結果と外部基準データで共通に使うスペクトル CSV schema を読み込む。
`FitCalibration.py` では、この指標を使って簡易校正を行い、`--reference-csv` を指定した場合は基準スペクトルから target peak / FWHM / gain を抽出する。
`CompareSpectrum.py` では、同じ CSV schema にそろえた計算結果と Lumerical / 実測スペクトルを読み込み、TE/TM ごとの指標差分と RMSE を出力する。
`MQWGainSweep.py` でも `--reference-csv` を指定でき、この場合は各 sweep 点を基準スペクトルと比較し、RMSE が最小の sweep 値を JSON に保存する。

`FitCalibration.py` の初期実装は、次の順序で 1 次元の簡易 fit を行う。

```text
Eg_offset_well_eV -> broadening_eV -> gain_scale_cm
```

まず井戸層バンドギャップ補正で target polarization の peak wavelength を合わせ、次に線幅で FWHM を合わせ、最後に `gain_scale_cm` で peak gain を合わせる。
これは単一点ターゲットへの簡易 fit であり、複数キャリア密度、異なる井戸幅、または Lumerical / 実測スペクトル全体への外挿性を保証しない。
生成された校正 JSON は、必ず別条件で検証してから使う。
その検証では、`CompareSpectrum.py` で peak だけでなくスペクトル形状の RMSE も確認する。
...
10. k.p ゲイン計算で、MQW stack を z-grid に展開し、`k_t` ごとに電子・正孔サブバンドを求める。
11. キャリア密度から擬フェルミ準位を解き、TE/TM 遷移強度と線幅関数を使って材料ゲインスペクトルを計算する。
12. 必要に応じて校正 JSON、sweep、可視化、fit を適用する。
...
このデータベースは、校正値ではなく、一般的な III-V 材料パラメータ表に基づくスターター値である。
...
| name | explanation |
| --- | --- |
| `MQWProfile` | k.p 計算用に z-grid へ展開した MQW profile。バンド端、ポテンシャル、有効質量、Luttinger parameter を配列として持つ。 |
| `z_nm` | k.p 計算の成長方向座標 [nm]。 |
| `V_e_eV` | 電子用の相対ポテンシャル [eV]。 |
| `V_hh_eV` | HH 正孔用の相対ポテンシャル [eV]。 |
| `V_lh_eV` | LH 正孔用の相対ポテンシャル [eV]。 |
| `me_z` / `me_xy` | 電子の成長方向・面内有効質量。 |
| `mhh_z` / `mlh_z` | HH/LH の成長方向有効質量。 |
| `mhh_xy` / `mlh_xy` | HH/LH の面内有効質量。 |
| `KpSubbands` | `k_t` ごとの電子サブバンドと正孔サブバンドをまとめた構造。 |
| `kt_nm` | 面内波数 [nm^-1]。 |
| `ScalarState` | 電子サブバンドの固有値と波動関数。 |
| `ValenceState` | HH/LH 混合正孔サブバンドの固有値、HH 成分、LH 成分、成分重み。 |
| `hh_weight` / `lh_weight` | 正孔固有状態に含まれる HH/LH 成分の規格化重み。 |
| `carrier_density_cm3` | 活性井戸体積で定義するキャリア密度 [cm^-3]。 |
| `quasi_fermi_e_eV` | 電子側の擬フェルミ準位 [eV]。現行モデルの相対エネルギー基準での値。 |
| `quasi_fermi_h_eV` | 正孔側の擬フェルミ準位 [eV]。現行モデルの相対エネルギー基準での値。 |
| `broadening_eV` | Lorentzian または Gaussian 線幅 FWHM [eV]。 |
| `line_shape` | スペクトル線幅関数。`lorentzian` または `gaussian`。 |
| `gain_scale_cm` | 絶対ゲイン値を校正する経験的スケール。未校正では絶対値として扱わない。 |
| `gain_TE_cm-1` / `gain_TM_cm-1` | TE/TM 材料ゲイン [cm^-1]。 |
| `calibration` | 校正 JSON と CLI override を解決した結果。出力 JSON に保存される。 |
| `Eg_offset_well_eV` | 井戸層の無ひずみバンドギャップに加える校正オフセット [eV]。 |
| `Eg_offset_barrier_eV` | 障壁層の無ひずみバンドギャップに加える校正オフセット [eV]。 |
| `sweep` | `MQWGainSweep.py` の sweep 条件。sweep 名、値、上書き対象の校正フィールドを含む。 |
| `SpectrumMetrics` | peak wavelength、peak energy、peak gain、FWHM をまとめた指標。 |
| `peak_wavelength_nm` | 指定偏光のピークゲイン波長 [nm]。 |
| `peak_energy_eV` | 指定偏光のピークゲインエネルギー [eV]。 |
| `peak_gain_cm` | 指定偏光のピーク材料ゲイン [cm^-1]。 |
| `fwhm_meV` | 指定偏光ゲインピークの半値全幅 [meV]。 |
| `spectrum_rmse` | 同じ波長軸へ補間した 2 つのスペクトル間の RMSE。Lumerical / 実測比較用の基本指標。 |
| `FitState` | `FitCalibration.py` が求めた `Eg_offset_well_eV`、`broadening_eV`、`gain_scale_cm` と達成指標。 |
| `fit_result` | 生成された校正 JSON に保存される fit 結果。達成値と residual を含む。 |
| `residuals` | fit target に対する達成値の差。peak wavelength、peak gain、FWHM について保存する。 |
| `CompareSpectrum.py` | 計算 CSV と基準 CSV を読み、peak / FWHM / RMSE の差分 JSON と比較 PNG を出力する CLI。 |
| `predicted` / `reference` | 比較対象の計算スペクトルと基準スペクトル。CSV schema は `energy_eV`、`wavelength_nm`、`gain_TE_cm-1`、`gain_TM_cm-1`。 |
| `rmse_gain_cm` | 指定偏光で、基準波長軸へ補間した計算 gain と基準 gain の二乗平均平方根誤差 [cm^-1]。 |
| `reference_comparison` | `MQWGainSweep.py --reference-csv` で保存される sweep 点ごとの基準比較 metadata。RMSE 最小点を `best` として保持する。 |
