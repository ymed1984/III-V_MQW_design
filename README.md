...
uv run python -B src/MQWGainDesign.py \
  ...
  --band-plot out/band_diagram.png \
  --wavefunction-plot out/wavefunctions.png \
  --dispersion-plot out/subband_dispersion.png
```
...
| `--band-plot` | バンド端プロファイル PNG 出力先 | なし |
| `--wavefunction-plot` | `k_t=0` 波動関数 PNG 出力先 | なし |
| `--dispersion-plot` | サブバンド分散 PNG 出力先 | なし |

出力 CSV には `energy_eV`, `wavelength_nm`, `gain_TE_cm-1`, `gain_TM_cm-1` が保存されます。
追加プロットは、バンド端、波動関数の閉じ込め、サブバンド分散の妥当性確認に使います。
